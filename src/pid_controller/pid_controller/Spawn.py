...
class MoveTurtle(Node):
    ...
    def spawn_turtle_at_random_location(self):
        ...
        if future.result() is not None:
            self.get_logger().info(f"Spawned turtle2 at ({x}, {y}) with orientation {theta}")
        else:
    ...
    def cmd_vel_callback(self, msg):
        ...
        vel_publisher = self.create_publisher(Twist, '/turtle2/cmd_vel', 10)
